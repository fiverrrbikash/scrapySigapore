...
class selenium():
    pageno = 2
    pages_to_scrape = input('how many pages to scrape?')
    email = input('email id?')
    password_of = input('password?')
    ...
    def nextpage(self):
        ...
        url = ('https://www.crunchbase.com/lists/vs-01/90e26de7-5cc5-4f94-af50-ee3b6e24f6e9/funding_rounds?pageId='+str(self.pageno)+'_a_97c47ad0-d04e-42e3-971d-c80953fb6ade')
        if (self.pageno<= int(self.pages_to_scrape)):
            self.pageno+=1
            self.driver.get(url)
    ...
    def __init__(self):
        ...
        email_storage.send_keys(self.email)
        time.sleep(2)
        password.send_keys(self.password_of)
        password.submit()
        time.sleep(4)
